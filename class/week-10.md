...
운영 환경에서는 `SqliteSaver`나 `PostgresSaver`를 쓰지만, 처음 구현은 메모리 기반으로 시작한다.
...
9주차에서 만든 루트 가상환경과 `multi-agent/` 폴더를 그대로 사용한다.
...
## 10.5 수업 실습: Supervisor 예시 구현

수업 예시는 **연구자 → 분석가 → 작성자** 구조를 Supervisor가 동적으로 라우팅하는 시스템이다. 완성 예시는 아래 파일에 정리한다.

```text
multi-agent/docs/week10_inclass_supervisor.md
```

Homework는 같은 구조를 다른 주제로 바꾸어 설계·구현한다.

```text
multi-agent/docs/week10_homework_supervisor.md
```

### 수업 예시 입력

```markdown
질문: 2025년 LangGraph 1.0의 주요 변경점을 학부생이 이해할 수 있게 정리해줘.
```
...
이때 무한 루프가 나면 **재시도 횟수 제한**이 왜 필요한지 확인할 수 있다.
...
`recursion_limit`은 LangGraph가 무한 루프를 막는 안전장치다. 처음 구현에서는 8~12 정도로 둔다.
