...
GitHub Copilot이 이 저장소에서 항상 참고할 기본 규칙이다.

```markdown
# Copilot Instructions

이 프로젝트는 수업용 일정공지 에이전트이다.

## 작업 원칙
- 코드는 가능한 한 단순하게 작성한다.
- 복잡한 프레임워크를 새로 추가하지 않는다.
- 학생이 읽을 수 있도록 함수 이름을 명확하게 쓴다.
- 실행 방법을 README 또는 주석에 남긴다.
- 변경 후에는 실행 명령과 확인 결과를 보고한다.

## 금지
- 사용자가 요청하지 않은 패키지를 설치하지 않는다.
- 기존 파일을 크게 갈아엎지 않는다.
- API 키나 비밀값을 코드에 넣지 않는다.
```
...
코드는 아직 만들지 말고, todo.md를 학생용 단계로 다듬어줘.
```
...
| 10~25분 | Copilot 실습: instruction, context, todo 만들기 |
| 25~45분 | Gemini CLI 실습: 계획 먼저 받기, todo 수정 요청 |
| 45~65분 | Antigravity 실습: Agent Manager 작업 만들기 |
| 65~80분 | 같은 요청을 세 도구에 넣고 결과 비교 |
